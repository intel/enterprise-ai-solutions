...
NRI CPU balloons pin inference pods to a set of CPUs on a NUMA node so they do
not share cores with noisy neighbors. This page shows how to enable the policy
and how to deploy models that use it.
...
| See how NUMA-aware CPU placement fits the broader architecture | [Architecture & Design](../meet/architecture.md) |
